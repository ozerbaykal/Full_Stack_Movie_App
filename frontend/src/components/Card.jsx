    ...
    >
      <div className="relative">
        ...
        <span
          ...
          className="absolute right-[-10px] top-[-10px] rounded-full text-white font-semibold p-1 sm:p-2 bg-[#d7d727] "
        >
          {Number(movie.rating).toFixed(1)}
        </span>
      </div>
      <div className="flex flex-col justify-between h-[full]">
        <h3 className="font-bold text-xl md:text-2xl md:mt-4 line-clamp-2">
          {movie.title}
        </h3>
        <div>
          <p>{movie.year}</p>
          ...
          <p className=" bg-red-400 rounded-md py-1 px-2 w-fit  text-white mt-2">
            {movie.language}
          </p>
        </div>
      </div>
